...
export default {
  ...
  methods: {
    ...
    async login (_username, _password) {
      var e
      if (_username === '') {
        e = new Error('Please insert your username')
        e.name = 'UserError'
        throw e
      }
      ...
      */
      ...
      var jsonMetadata = JSON.parse(account.json_metadata)
      var keyFound = false
      var typeOfPassword = ''
      var auth = {
        user: '',
      ...
      if (!keyFound) {
        e = new Error('Incorrect password. Please use posting key or WIF')
      ...
      auth.logged = true
      auth.user = _username
      auth.imgUrl = Utils.extractUrlProfileImage(jsonMetadata)
